...
public class NewPassActivity extends AppCompatActivity {
    ...
    public void saveButtonClicked(View view){
        //getting values from ui user input
        ...
        String q2="";
        String q3="";
        //sets security answers if checkbox is clicked
        if(secQ){
        ...
        int numLetters=0;
        //tries to get number input for password length
        try{
            String nums = numberCharText.getText().toString();
            numLetters = Integer.parseInt(nums);
        }catch(Exception e){//catches exception for invalid entry
        ...
        //checks to verify that an account name was input
        if(name.equals("")){
            Toast.makeText(NewPassActivity.this,"Please name the account.", Toast.LENGTH_SHORT).show();
        }else if(numLetters < 8){//checking if the password length is within range
        ...
        }//checks to see if the name of account is already used
        ...
        }else{//we gucci for account creation
            Password password = passwordOnly.addPassword(name,numLetters,capLetters,specialChars,numbers);//gets random password with input params
            password.setSecurityAnswers(q1);
            password.setSecurityAnswers(q2);
            password.setSecurityAnswers(q3);
            dbHandler.addAccount(password, passphrase);//adds account
